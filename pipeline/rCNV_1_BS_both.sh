...
if [ ${fqnum} -gt 0 ]; then
        ...
        Rscript ../../0.scripts/bowtie_findex_both.R
...
# bowtie2 align
R1_ind=`sed -n "1p" R1_rlt.txt`
R2_ind=`sed -n "1p" R2_rlt.txt`
...
do
	...
	bowtie2 --very-sensitive-local -p ${Threads} -x ../bowtie_ref/${RID}_ref -1 ${R1_ind} -2 ${R2_ind} -S ${RID}.sam
